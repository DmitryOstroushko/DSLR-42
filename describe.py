			...
			for percent in [25, 50, 75]:
				...
				for idx in range(len(features)):
					if numerical_features[features[idx]]:
						data = np.array(dataset[:, idx], dtype=float)
						data = data[~np.isnan(data)]
						print(f' |{function_list[function](data, percent):>12.4f}', end = '')
				print()
		else:
...
def do_main_function():
	"""
	Main function of DSLR project
	"""
	parser = argparse.ArgumentParser()
	parser.add_argument("dataset", type=str, help="A name for input dataset")
	...
	if args.print_mode == 'v':
		describe_vertical(args.dataset)
	elif args.print_mode == 'h':
		describe_horizontal(args.dataset)
	else:
